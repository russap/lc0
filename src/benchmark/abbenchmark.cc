...
void AbBenchmark::Run() {
  ...
  try {
    ...
    //nnue_init("21-05-16-g-nn-8a08400ed089.nnue");
    // start position
    int score = nnue_evaluate_fen(
        "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
    printf("test evaluate: start position: %.3f\n", score/1000.0);
    ...
    for (std::string position : testing_positions) {
      ...
      auto search = std::make_unique<Search>(
          ...
          std::make_unique<CallbackUciResponder>(
              std::bind(&AbBenchmark::OnBestMove, this, std::placeholders::_1),
              std::bind(&AbBenchmark::OnInfo, this, std::placeholders::_1)),
...
void AbBenchmark::OnBestMove(const BestMoveInfo& move) {
  std::cout << "bestmove " << move.bestmove.as_string() << std::endl;
}

void AbBenchmark::OnInfo(const std::vector<ThinkingInfo>& infos) {
