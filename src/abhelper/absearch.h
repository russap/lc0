...
class AlphaBetaSearch {
 public:
  AlphaBetaSearch(const NodeTree& tree, Network* network,
 ...
 private:
  ...
  friend class AlphaBetaSearchWorker;
};

// Single thread worker of the search engine.
// That used to be just a function Search::Worker(), but to parallelize it
// within one thread, have to split into stages.
class AlphaBetaSearchWorker {
 public:
  AlphaBetaSearchWorker(AlphaBetaSearch* search, const SearchParams& params, int id)
      : search_(search),
  ...
  void RunBlocking() {
    LOGFILE << "Started alpha beta search thread.";
    try {
